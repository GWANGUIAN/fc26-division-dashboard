...
- 게시글에 이미지가 여러 장이면(예: "이전 전적"과 "현재 전적" 스크린샷을 함께 올리는 경우) 각각 시도해 유효하게 읽힌 것 중 **총 전적(승+무+패)이 더 큰 쪽**을 채택한다(`chooseRecord`, `src/shared/record-extraction.ts`). 총 전적까지 같은데 승/무/패 구성이 다른 경우(진짜 모순)만 `PromotionPost.recordNeedsReview`를 세워 사람이 볼 수 있게 한다.
...
- 스코프 게시판은 `[내가 직접 홍보]`, `[응원버튜버홍보]` 카테고리만 후보 활동으로 반영한다.
