...
- **결전·엔딩**: `m-90-finale`은 스타디움에서만 연 월드 모달의 `onRoundEnd`를 받아 합10 70점 → 공 튀기기 20회 → 카드 짝 맞추기 17턴 이하를 순서대로 판정한다. 심판·제초왕 컷, 관중석의 플레이어 외 10명 정적 스프라이트·응원, 코어 정지/황금 잔디 개화, 기존 `GroupPhotoOverlay`, 크레딧 카드까지 연결했다. 완료 시 `ending-seen`·`area-weed-open`·`weed-restored`가 저장된다.
