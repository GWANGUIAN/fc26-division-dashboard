...
- **결전·엔딩**: `m-90-finale`은 스타디움에서만 연 월드 모달의 `onRoundEnd`를 받아 합10 80점 → 공 튀기기 20회 → 카드 짝 맞추기 17턴 이하를 순서대로 판정한다. 심판·제초왕 컷, 관중석의 플레이어 외 10명 정적 스프라이트·응원, 코어 정지/황금 잔디 개화, 기존 `GroupPhotoOverlay`, 크레딧 카드까지 연결했다. 완료 시 `ending-seen`·`area-weed-open`·`weed-restored`가 저장된다.
...
- `state/daily.ts`: KST 날짜 문자열로 결정적 셔플, 매일 3개. 풀은 합10 40/70점·공튀기기 10/20회·프리킥 3/5골·러시 300/600m·카드 짝 맞추기 24턴 이내·오락실 서로 다른 게임 3종·월드 카드 2장 공개·서로 다른 주민 3/6명(13종). 같은 게임의 난이도 변형은 `group`으로 묶어 한 날에 함께 뽑지 않는다. 소진되는 황금 공은 제외. 엔딩 후 활동 자동 기록, 광장 (44,25) 게시판에서 3개 달성 후 하루 한 번 수령. 열린 UI는 1초마다 날짜 갱신, 수령 시 날짜 재검사, 과거 날짜로 시계가 되돌아가면 진행/수령 차단. `daily.stamps` 날짜 이력 보존 및 7/14/30 뱃지를 동일 저장에 기록. 날짜 경계·중복 클릭·재접속·시계 역행 테스트.
