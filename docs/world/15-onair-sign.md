...
- **세로**: 문턱 행 아래 가장자리에서 **72px 위**가 전광판의 아래 가장자리(`ON_AIR_RISE`, 고정). 본인 집에 뜨는 **"○○의 집" 이름표는 그 위**(전광판 위 4px 띄워서, 문턱 위 123~109px)에 쌓인다. 전광판이 이름표를 피해 가는 게 아니라 이름표가 전광판 위로 올라간 것이다. 배치 상수는 전부 [`engine/doorSigns.ts`](../../src/web/world/engine/doorSigns.ts)에 있다.
- **깊이**: 건물의 마지막 그릴 조각(strip) 직후에 그려서 벽과 같이 정렬된다(플레이어가 집 뒤로 가면 가려지고, 앞을 지나면 위로 보인다).
- 이름표("○○의 집")도 **같은 발판 가운데 축**이다(`homeSignAnchor`). 폭을 짝수로 맞춰 그려서 가운데가 정수 픽셀에 놓인다. 이름표는 본인 집에만 뜨고, 전광판은 11채 모두에 있다.
...
| 바꾸고 싶은 것 | 고칠 곳 |
| --- | --- |
| 전광판 높이(문·지붕과의 거리) | `doorSigns.ts` `ON_AIR_RISE` (이름표는 자동으로 따라 올라감. 집마다 다르게 하려면 `findOnAirSigns`에 오프셋 표 추가) |
| 이름표와 전광판 사이 간격 | `doorSigns.ts` `HOME_SIGN_GAP` |
...
2. 11채 모두 전광판 가운데가 발판 가운데와 같은 세로선 위에 있고, 본인 집에서는 "○○의 집" 이름표가 전광판 바로 위에 같은 세로선으로 정렬된다(겹치지 않는다).
