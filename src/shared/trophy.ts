...
function promotionPostsFor(streamer: StreamerRecord): PromotionPost[] {
  if (streamer.promotionHistory?.length)
    return chronological(streamer.promotionHistory);
  ...
  return chronological([
    ...
    ...(streamer.lastPost ? [streamer.lastPost] : []),
  ]);
}
...
export function buildTrophyAwards(streamers: StreamerRecord[]): TrophyAwards {
  ...
  const bestDailySteps = Math.max(
    0,
    ...dailyRecords.map((record) => record.steps),
  );

  const divisionOneCandidates = streamers
    .flatMap((streamer) => {
      const reachedAt = promotionPostsFor(streamer).find(
        (post) => post.division === 1,
      )?.publishedAt;
    ...
    .sort(
      (left, right) =>
        ...
        left.streamer.displayName.localeCompare(
          right.streamer.displayName,
          "ko",
        ),
    )
    .slice(0, 3)
  ...
  const promotionRecords = streamers.map((streamer) => {
    ...
    return {
      ...
      scopeCount,
      elevenVsElevenCount,
      totalCount: scopeCount + elevenVsElevenCount,
    };
  });
  const bestPromotionCount = Math.max(
    0,
    ...promotionRecords.map((record) => record.totalCount),
  );
  ...
  const mostGames = Math.max(
    0,
    ...matchRecords.map((record) => record.totalGames),
  );

  const winRateRecords = streamers.flatMap((streamer) => {
    const winRate = streamer.record
      ? winRatePercent(streamer.record)
      : undefined;
  ...
  return {
    dailyPromotion:
      bestDailySteps > 0
        ? dailyRecords.filter((record) => record.steps === bestDailySteps)
        : [],
    divisionOne: divisionOneCandidates,
    selfPromotion:
      bestPromotionCount > 0
        ? promotionRecords.filter(
            (record) => record.totalCount === bestPromotionCount,
          )
        : [],
    mostMatches:
      mostGames > 0
        ? matchRecords.filter((record) => record.totalGames === mostGames)
        : [],
    bestWinRate:
      bestRate >= 0
...
export function trophyBadgesFor(
  streamer: StreamerRecord,
  awards: TrophyAwards,
): TrophyBadge[] {
  const badges: TrophyBadge[] = [];
  const divisionOne = awards.divisionOne.find(
    (award) => award.streamer.id === streamer.id,
  );
  if (divisionOne)
    badges.push({
      key: "division-one",
      name: DIVISION_ONE_LABEL[divisionOne.rank],
      emoji: DIVISION_ONE_EMOJI[divisionOne.rank],
    });
  if (awards.mostMatches.some((award) => award.streamer.id === streamer.id))
    badges.push({ key: "most-matches", name: "최다 경기 출전", emoji: "⚔️" });
  if (awards.bestWinRate.some((award) => award.streamer.id === streamer.id))
    badges.push({ key: "best-win-rate", name: "최고 승률", emoji: "👑" });
  if (awards.dailyPromotion.some((award) => award.streamer.id === streamer.id))
