...
function StreamerCard({ streamer, onOpen }: { streamer: StreamerRecord; onOpen: () => void }) {
  return <button className="streamer-card" onClick={onOpen} aria-label={`${streamer.displayName} 상세 보기`}>
    <Avatar {...streamer} />
    <span className="streamer-card__copy"><strong>{streamer.displayName}</strong><SoopTags tags={streamer.soopTags} /><small>{streamer.lastPost ? formatDate(streamer.lastPost.publishedAt) : "첫 보고 대기"}</small></span>
...
function CafeLink({ href }: { href: string }) { return <a className="action cafe" href={href} target="_blank" rel="noreferrer"><i>{cafeIcon}</i> 네이버 카페 원문</a>; }
...
function EvaluationCard({ application, onOpen }: { application: OneVsOneApplicationView; onOpen: () => void }) {
  ...
  return <article className={`evaluation-card ${result ? "evaluation-card--completed" : ""}`}>
    <button className="evaluation-card__main" onClick={onOpen} aria-label={`${application.displayName} 1대1 평가 상세 보기`}><Avatar {...application} /><span><strong>{application.displayName}</strong><SoopTags tags={application.soopTags} /><small>{application.cafeAuthor} · 신청 {formatDate(application.publishedAt)}</small></span><b className={`evaluation-status ${result ? "done" : "waiting"}`}>{result ? "대결 완료" : "대기 중"}</b></button>
    {result && <button className="evaluation-result" onClick={onOpen}><span>{result.candidateScore} : {result.woowakgoodScore}</span><strong>{result.verdict}</strong><small>{formatDate(result.playedAt)}</small></button>}
    <div className="evaluation-card__actions"><CafeLink href={application.articleUrl} /></div>
  </article>;
}
...
function EvaluationModal({ application, onClose }: { application: OneVsOneApplicationView; onClose: () => void }) {
  ...
  return <Modal onClose={onClose} label="1대1 평가 상세"><div className="modal__identity"><Avatar {...application} /><div><span className="eyebrow">ONE VS ONE APPLICATION</span><h2>{application.displayName}</h2><SoopTags tags={application.soopTags} /><p>{application.cafeAuthor} · 신청 {formatDate(application.publishedAt)}</p></div></div>
    ...
    <div className="actions"><CafeLink href={application.articleUrl} />{application.soopId && <a className="action soop" href={soopChannelUrl(application.soopId)} target="_blank" rel="noreferrer">신청자 SOOP ↗</a>}<a className="action soop" href={soopChannelUrl(opponent.soopId)} target="_blank" rel="noreferrer">우왁굳 SOOP ↗</a></div>
  </Modal>;
}
...
export function App() {
  ...
  useEffect(() => { loadSnapshot().then(setSnapshot).catch((caught) => setError((caught as Error).message)); }, []);
  const streamers = useMemo(() => (snapshot?.streamers ?? []).filter((streamer) => searchable(streamer.displayName, streamer.cafeAliases, query) && (mode === "all" || (mode === "auto" ? streamer.overridePolicy === "auto" : streamer.overridePolicy !== "auto"))), [snapshot, query, mode]);
  ...
  return <main>
    ...
    <section className="hero" id="top"><div><p className="eyebrow">FC26 · {isDivision ? "SEASON DIVISION BOARD" : "ONE VS ONE EVALUATION"}</p><h1>{isDivision ? <>잰디 <mark>동아리 후보</mark><br />대시보드</> : <>1:1 <mark>평가 신청</mark><br />현황</>}</h1><p className="intro">{isDivision ? "카페에 보고된 FC26 디비전 승격 현황을 추적합니다." : "1대1 평가 신청 게시글과 운영자가 등록한 대결 결과를 표시합니다."}</p></div><div className="sync"><span className={`sync-dot ${snapshot?.status === "degraded" ? "bad" : ""}`} /> <b>{snapshot?.status === "degraded" ? "데이터 갱신 확인 필요" : "3 MINUTE REFRESH"}</b><small><span className="refresh-icon" aria-hidden="true">↻</span> 3분마다 갱신 · {snapshot ? `${formatDate(snapshot.generatedAt)} 기준` : "데이터 연결 중"}</small></div></section>
    {(error || snapshot?.status === "degraded") && <div className="warning">{error ?? snapshot?.message ?? "마지막 수집에 문제가 있어 이전 데이터를 표시합니다."}</div>}
    <section className="controls" aria-label={isDivision ? "디비전 필터" : "평가 신청 필터"}><label><span className="sr-only">검색</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="이름 또는 카페 닉네임 검색" /></label><div className="segmented">{isDivision ? (["all", "auto", "manual"] as const).map((value) => <button key={value} className={mode === value ? "active" : ""} onClick={() => setMode(value)}>{value === "all" ? "전체" : value === "auto" ? "자동 갱신" : "수동 보정"}</button>) : (["all", "pending", "completed"] as const).map((value) => <button key={value} className={evaluationFilter === value ? "active" : ""} onClick={() => setEvaluationFilter(value)}>{value === "all" ? "전체" : value === "pending" ? "대기 중" : "대결 완료"}</button>)}</div></section>
