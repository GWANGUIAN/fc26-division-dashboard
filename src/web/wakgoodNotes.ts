...
export interface WakgoodNoteGroup {
  ...
  notes: string[];
  /**
   * 이 그룹에 해당하는 SOOP VOD 다시보기 링크(들). 특정 구간(그 선수 차례)부터
   * 바로 재생되도록 타임스탬프가 포함된 URL을 그대로 넣으면 된다. 재평가
   * 회차의 다시보기 링크가 아직 없으면 생략하고, 나중에 생기면 그 그룹에만
   * 추가하면 된다(여러 번 평가받았어도 링크가 하나뿐이면 보통 첫 번째
   * 그룹에 몰아서 넣는다).
   */
  vodUrls?: string[];
}
...
function notes(
  vodUrls: string[] | undefined,
  ...noteTexts: string[]
): WakgoodNoteGroup[] {
  return [{ notes: noteTexts, vodUrls }];
}
...
export const WAKGOOD_NOTES: Record<string, WakgoodNoteEntry> = {
  ...
  janine95kim: {
    noteGroups: notes(
      [noteVodUrl(4359)], // 01:12:39
      "(넘어감)",
    ),
  }, // 재닌
  been11060: {
    noteGroups: notes(
      [noteVodUrl(4362)], // 01:12:42
      "오프더볼 낫배드",
      "로빙스루 없음",
    ),
  }, // 오구
  lina0108: {
    noteGroups: notes(
      ...
      [noteVodUrl(5064), noteVodUrl2(16240)], // 01:24:24, 04:30:40
      "오버래핑 오프더볼 좋음",
      "볼배급 괜찮은데 너무 약하게 참",
      "전방 수비 리스크 너무큼. 양학 용.",
  ...
  kaksjak0730: {
    noteGroups: [
      ...
      },
      {
        label: "2트",
        notes: [
          "쇄도가 생기셨음. 그래서 몇번은 좋았느데 몇번은 쇄도 해도 의미없는데 너무 수비 많은 곳에 억지로 쇄도하는 경우가 많았음.",
          "그리고 상대 압박 생기니까 확실히 볼키핑 이후에 볼배급하는 컨트롤 적인 면은 확실히 잘하심",
...
/**
 * 실제로 보여줄 내용이 있는 그룹만 남긴다. 나중에 채울 회차를 미리
 * 만들어둔 자리표시자(notes가 비어 있거나 빈 문자열만 있는 그룹)는
 * 아직 존재하지 않는 것처럼 취급해서, 화면에서도 라벨/구분선 없이
 * 조용히 숨겨진다.
 */
export function visibleWakgoodNoteGroups(
  noteGroups: WakgoodNoteGroup[] | undefined,
): WakgoodNoteGroup[] {
  return (noteGroups ?? []).filter((group) =>
    group.notes.some((note) => note.trim().length > 0),
  );
}
...
export function flattenWakgoodNotes(
  ...
  return visibleWakgoodNoteGroups(noteGroups).flatMap((group) => group.notes);
}

/** 그룹으로 나뉜 다시보기 링크를 하나의 배열로 펼친다 (엑셀 내보내기 등에 사용). */
export function flattenWakgoodVodUrls(
  noteGroups: WakgoodNoteGroup[] | undefined,
): string[] {
  return noteGroups?.flatMap((group) => group.vodUrls ?? []) ?? [];
}
