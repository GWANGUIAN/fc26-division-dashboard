...
export async function loadSnapshot(): Promise<DashboardSnapshot> {
  ...
  // Rebuilt on every call (not hoisted to a module-level constant) so that
  // clicking refresh in local dev visibly bumps generatedAt, the same way a
  // real API response would.
  if (!url) return buildDemoSnapshot();
