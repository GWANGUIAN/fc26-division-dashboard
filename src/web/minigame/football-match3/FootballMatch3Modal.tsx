...
export default function FootballMatch3Modal({ onClose, onRoundEnd }: { onClose: () => void; onRoundEnd?: (result: FootballMatch3RoundResult) => void }) {
  ...
  const newGame = () => { ranking.abandon(); start(); };
  // The last move ends the round before its animation has played out: that round is already decided, so it cannot be dropped.
  const canGiveUp = game.state.phase === "playing" && game.state.engine.phase === "playing";
