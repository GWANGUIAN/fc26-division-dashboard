...
**두 드래그 종류는 시작값만 다르고, 이후엔 동일한 히스테리시스를 따른다:**
...
- **후보 카드 드래그**: `true`(열림)로 시작 — 서랍을 호버해서 열어둔 상태에서 카드를 잡았다고 가정.

두 경우 모두, 일단 열린 뒤에는 실제 열린 서랍 높이(`DRAWER_EXPANDED_FRACTION = 0.46`, CSS `46vh`와 동일해야 함) 선을 벗어나야만 닫히고, 일단 닫힌 뒤에는 다시 접힌 서랍의 좁은 띠(132px)까지 내려와야만 열린다. 처음엔 "피치 카드 드래그로 서랍이 열린 뒤"에는 단일(낮은 132px) 기준으로만 닫히게 해뒀었는데, 그러면 서랍이 열리자마자 살짝만 움직여도 바로 닫혀버려서 — 열림/닫힘 두 기준을 **어느 상태에서 시작했든 항상 같은 히스테리시스**로 통일했다. 두 기준을 아예 하나로 합치려던(대칭이 아니라 완전히 단일 값으로) 시도들은 실패했다 — 아래 "실패했던 접근들" 참고.
